    ...
    class TabOpen : Layer<UIManager>
    {
        ...
        class Grabbing : State<UIManager>
        {
            ...
            public override void OnStateUpdate()
            {
                ...
                if (SystemInfo.deviceType == DeviceType.Handheld ? Input.GetTouch(origin.grabTouchIndex).phase == UnityEngine.TouchPhase.Ended : SystemInfo.deviceType == DeviceType.Desktop ? Input.GetMouseButtonDown(0) : true)
                {
                    if (tmp.gameObject != null)
                    {
                        ...
                        if (grabbingSlot.count > 0)
                        {
                            if(SystemInfo.deviceType == DeviceType.Handheld)
                            {
                                ...
                                if(grabbingSlot.count > 0)
                                {
                                    ...
                                    if (grabbingSlot.count > 0)
                                    {
                                        origin.player.DropItem(grabbingSlot.item, grabbingSlot.count);
                                    }
